...
- **Filtering** — search, namespace picker, failures-only (real failures), progressing-only (in-flight reconciles), waiting-only (stale resources), and drifting-only Terraform resources
...
| `t` | Toggle drifting-only filter (currently detected drift; Terraform only) |
